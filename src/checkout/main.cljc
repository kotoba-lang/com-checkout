(ns checkout.main
  "Kotodama WASM entrypoint for the Checkout clean-room actor (L4) — Clojure port.
  ...
  facts (`checkout.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "checkout")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"checkout.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "checkout-compat" :tier tier :entities entities} 200])
...
(defn start! [] :checkout-compat/ready)
